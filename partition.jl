...
"""
function hash_form_array(p::Array)
...
struct Partition
    """
    :param upper_points: Upper points of partitions as list
    :param lower_points: Lower points of partition as list
    """
    upper_points::Array{Int64, 1}
    lower_points::Array{Int64, 1}

    function Partition(upper_points::AbstractArray, lower_points::AbstractArray, hash_form::Bool = true)
        if hash_form
            (upper_points, lower_points) = hash_form_array([Int64.(upper_points), Int64.(lower_points)])
        end
...
"""
function helper_new_point_values(p::Partition, q::Partition)::Partition
...
- `p` with consisten form
"""
function hash_form(p::Partition)::Partition
...
- `p` tensor product `q`
"""
function tensor_product(p::Partition, q::Partition)::Partition
...
This function applies an involution on `p` (in O(n) because hash_form, else O(1)).
...
- involution of `p`
"""
function involution(p::Partition)::Partition
...
This function applies an vertical reflection on `p` (in O(n) because hash_form, else O(1)).
...
"""
function vertical_reflection(p::Partition)::Partition
...
This function applies a rotation on `p` (in O(n) because hash_form, else O(1)). 
...
- rotation of `p`
"""
function rotation(p::Partition, lr::Bool, tb::Bool)::Partition
...
- `p` composition `q`
"""
function composition(p::Partition, q::Partition)::Partition
...
function get_trace(trace::Dict, start::Partition)
    """track the trace with breath first search"""

    if !(start in keys(trace))
        print("Partition $(start) not found in trace")
    end
...
@time construct_category([Partition([1, 2], [2, 1])], 8)
